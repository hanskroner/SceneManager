...
//  Adapted from Rob Napier's gist here:
...
public enum JSON: Codable, CustomStringConvertible {
    ...
    public func prettyPrint(indentation: Int = 0) -> String {
        switch self {
        case .string(let string):
            return "\"\(string)\""
        case .number(let double):
    ...
    public struct Key: CodingKey, Hashable, Comparable, CustomStringConvertible {
        public static func < (lhs: JSON.Key, rhs: JSON.Key) -> Bool {
